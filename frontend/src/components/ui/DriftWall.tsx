...
const DEFAULT_ITEMS = Array.from({ length: 15 }, (_, i) => {
  const names = ["Alice", "Bob", "Charlie", "David", "Eve", "Frank", "Grace", "Heidi", "Ivan", "Judy", "Mallory", "Nina", "Oscar", "Peggy", "Trent"];
  const comments = [
    "This is absolutely amazing! 🔥",
    "I've never seen anything like this before.",
    "Mind blowing work, keep it up!",
    "Can't wait to see what you build next.",
    "This totally changed my workflow.",
    "Best digital product I've used this year.",
    "Speechless. The animations are so smooth.",
    "10/10 would recommend to anyone.",
    "Such clean code and architecture.",
    "Incredible attention to detail.",
    "This is exactly what I was looking for.",
    "The UI/UX is simply phenomenal.",
    "Game changer for my projects.",
    "Absolutely stunning visuals.",
    "Great job on the performance optimization!"
  ];
  return {
...
const DriftWall = ({
  ...
  const handlePointerMove = useCallback(
    (e: any) => {
      ...
      activeIdRef.current = id as string;
  ...
  const renderTile = (item: any, id: string, colIndex: number) => {
    const inner = (
      <span className="drift-wall__inner bg-[#111] border border-white/5 p-4 flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <img src={item.avatar} alt={item.name} className="w-9 h-9 rounded-full object-cover" />
          <div className="flex flex-col">
            <span className="text-white text-sm font-semibold tracking-tight">{item.name}</span>
            <span className="text-zinc-500 text-xs">{item.handle}</span>
          </div>
        </div>
